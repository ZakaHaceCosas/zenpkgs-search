<script lang="ts">
    ...
    function isRowVisible(row) {
        ...
        if (
            row.classList.contains("virtual-search-result") ||
            row.classList.contains("virtual-row")
        ) {
            return true;
        }
        // Standard tree check
    ...
    class VirtualScroller {
        ...
        scrollToIndex(index: number) {
            ...
            this.scrollParent.scrollTo({
                top:
                    containerOffset +
                    itemTop -
                    this.scrollParent.clientHeight / 2 +
                    this.rowHeight / 2,
                behavior: "smooth"
            });
        }
        ...
        destroy() {
            if (this.scrollParent)
                this.scrollParent.removeEventListener("scroll", this.renderChunk);
    ...
    const newsNav = {
                ...
                authorsHtml = news.authors
                    .map((authorName) => {
                        const maintainerData =
                            state.data && state.data.maintainers
                                ? state.data.maintainers[authorName]
                                : null;
        ...
        goToArchive: () => {
            ...
            document
                .getElementById("news-page-archive-entry")
                .classList.remove("active", "exit-left");
    ...
    const settingsStore = localStorage.createInstance({ name: "settings" });
    const dataStore = localStorage.createInstance({ name: "registry_cache" });
    const aiStore = localStorage.createInstance({ name: "ai_response_cache" });
    ...
    const addToScratchpad = (path, category, specificType = null) => {
        ...
        const resolveType = (raw) => {
            ...
            if (typeof raw === "object") {
                ...
                if (raw.type)
                    return {
                        t: typeof raw.type === "string" ? raw.type : "complex",
                        e: raw.enum || []
                    };
                return { t: "complex", e: [] };
            }
    ...
    export const renderScratchpad = () => {
        ...
        const processValueString = (str) => {
            ...
            if (result.endsWith(";"))
                result = result.slice(0, -1) + `<span class="sp-token-semi">;</span>`;
            return result;
        };
    ...
    export const openBadgePopup = (el) => {
        ...
        if (left < 12) left = 12;
        if (left + popRect.width > panelRect.width - 12)
            left = panelRect.width - popRect.width - 12;
    ...
    export const getScratchpadContent2 = () => {
        ...
        const replaceTokenValue = (container, newToken) => {
            ...
            container.childNodes.forEach((node) => {
                ...
                // Inject the token string for widgets
                    ...
                    node.classList &&
                    (node.classList.contains("sp-bool-wrapper") ||
                        node.classList.contains("sp-type-badge"))
                ) {
                    line += escapedToken;
                }
            });
        ...
        const processLine = (div) => {
            if (div.querySelector(".sp-bool-wrapper")) {
                ...
                else if (badge.dataset.type === "list")
                    return replaceTokenValue(div, `__LIST:${val}__`);
    ...
    function getCursorPos(editor: Node) {
        ...
        while (
            node &&
            node !== editor &&
            (!node.classList || !node.classList.contains("sp-line"))
        ) {
                ...
                if (currentNode.nodeType === Node.TEXT_NODE && currentOffset === 0) {
                    ...
                    if (prev) {
                        ...
                        if (
                            ...
                            (prev.classList.contains("sp-type-badge") ||
                                prev.classList.contains("sp-bool-wrapper"))
                        ) {
                            e.preventDefault();
                            prev.remove();
                            state.scratchpadConfig = getScratchpadContent();
                        ...
                        if (
                            ...
                            if (
                                textTarget.nodeType === Node.TEXT_NODE &&
                                textTarget.textContent.length > 0
                            ) {
                    ...
                    if (type && path && state.data[type]) {
                        ...
                        if (isValid) {
                            setTimeout(() => {
                                ...
                                if (state.focusedRow)
                                    state.focusedRow.el.scrollIntoView({ block: "center" });
                            }, 100);
                        } else {
    ...
    export const updateVersionUI = () => {
        ...
        const label =
            state.selectedVersion === "latestCommit" ? "Latest Commit" : state.selectedVersion;
        ...
        categories.forEach((cat) => {
            ...
            if (cat.k === "maintainers") {
                ...
                keys.forEach((key) => {
                    ...
                    chip.onclick = (e) =>
                        showMaintainerPopup(key, data[cat.k][key], e.currentTarget);
                ...
                const scroller = new VirtualScroller(
                    ...
                    (row, item) => {
                        ...
                        if (
                            state.diffMode &&
                            state.diffBaseData &&
                            !state.diffBaseData[item.type][item.path]
                        ) {
                        ...
                        if (item.hasSub) {
                            ...
                            if (item.expanded) {
                                ...
                                const newItems = subKeys.map((k) => ({
                                    ...
                                    hasSub:
                                        item.data.sub[k].sub &&
                                        Object.keys(item.data.sub[k].sub).length > 0,
                                    parentId: item.path
                                }));
    ...
    function renderRecursiveTree(nodes, container, path, type, forceExpand = false) {
        ...
        allKeys.forEach((key) => {
            ...
            if (oldParentNode) {
                oldNode =
                    oldParentNode.sub && oldParentNode.sub[key]
            ...
            if (state.diffMode) {
                if (!node && oldNode) {
                    ...
                    const metaDiff =
                        JSON.stringify(cleanMeta(node.meta)) !==
                        JSON.stringify(cleanMeta(oldNode.meta));
                    if (metaDiff) {
            ...
            if (!row.title && displayNode.meta) {
                row.title = escapeHTML(
                    displayNode.meta.summary || displayNode.meta.description || ""
                );
            }
            ...
            if (!displayNode.meta && !hasSub)
                row.innerHTML += `<div class="meta-badge">NO META</div>`;
    ...
    export const cycleSearchMatches = (direction = 1) => {
        ...
        const findAndSelect = () => {
            const row = document.querySelector(
                `.virtual-row[data-path="${CSS.escape(item.path)}"]`
            );
            if (row) {
            ...
            if (catKey === "maintainers") {
                ...
                Object.keys(filtered)
                    ...
                    .forEach((key) => {
                        ...
                        chip.onclick = (e) =>
                            showMaintainerPopup(key, filtered[key], e.currentTarget);
                        container.appendChild(chip);
                    });
                root.appendChild(container);
            }
    ...
    export const navigateToPath = async (path, type) => {
        ...
        for (let i = 0; i < parts.length; i++) {
            ...
            const row = document.querySelector(
                `.tree-row[data-path="${safePath}"][data-type="${type}"]`
            );
        ...
        if (seq === kb.top) {
            ...
            if (searchActive && state.searchMasterIndex?.length > 0) {
                ...
                const rows = Array.from(
                    document.querySelectorAll(".tree-row, .virtual-row")
...
*Note: This is a local fallback.*
    ...
    function flattenTreeIterative(sourceData, type) {
        ...
        while (stack.length > 0) {
            ...
            const hasSub =
                node.sub && typeof node.sub === "object" && Object.keys(node.sub).length > 0;
    ...
    const collapseTree = () => {
        ...
        document
            .querySelectorAll(".row-icon.expanded")
    ...
    function renderCommandList(items, selectedIndex = 0) {
        ...
        if (historyItems.length > 0 && otherItems.length > 0 && !query) {
            finalDisplay = [
                ...historyItems,
                { separator: true, label: "All Commands" },
                ...otherItems
            ];
        } else {
        ...
        finalDisplay.forEach((cmd, idx) => {
            ...
            let rawKey =
                cmd.bindKey && state.keybinds[cmd.bindKey]
                    ? state.keybinds[cmd.bindKey]
                    : cmd.shortcut;
            ...
            div.onclick = () => {
                state.commandHistory = [
                    cmd.id,
                    ...state.commandHistory.filter((id) => id !== cmd.id)
                ];
                saveSettings();
    ...
    const smartScroll = (el) => {
        ...
        const scroller =
            el.closest(".main-content, .sidebar-content") ||
    ...
    function handleNav(action) {
            ...
            if (activeScroller) {
                ...
                if (action === "down") {
                    ...
                    if (currentIndex < items.length - 1) {
                        ...
                        const sIdx = state.activeScrollers.findIndex(
                            (s) => s.scroller === activeScroller
                        );
        ...
        // --- Standard Fallback ---
        const visibleRows = Array.from(document.querySelectorAll(".tree-row, .virtual-row")).filter(
            (r) => isRowVisible(r)
        );
        ...
        const currentEl =
            ...
            document.querySelector(
                `.virtual-row[data-path="${CSS.escape(state.focusedRow.path)}"]`
            );
    ...
    document.addEventListener("click", (e) => {
        ...
        const closeIfOutside = (id, triggerSelector) => {
            ...
            if (el.classList.contains("visible")) {
                if (
                    !e.target.closest(`#${id}`) &&
                    (!triggerSelector || !e.target.closest(triggerSelector))
                ) {
        ...
        closeIfOutside(
            "search-history",
            window.innerWidth < 768 ? "#search-bar-mobile" : "#search-bar"
        );
...
<div class="overlay-backdrop" id="modal-news">
    <div class="adw-window" style="width: 600px">
        ...
        <div class="window-content about-stack-container">
            ...
            <div
                id="news-page-archive-list"
                bind:this={I_NEWS_PAGE_ARCHIVE_LIST}
                class="stack-page off-right"
            >
                <div id="news-body-archive-list"></div>
            </div>

            <div
                id="news-page-archive-entry"
                bind:this={I_NEWS_PAGE_ARCHIVE_ENTRY}
                class="stack-page off-right"
            >
    ...
    <div class="adw-window">
        ...
        <div class="window-body">
            ...
            <div class="adw-group">
                ...
                <div
                    class="adw-row adw-switch-row"
                    onclick={toggleAdvancedScratchpad}
                    id="sp-mode-row"
                >
            ...
            <div class="adw-group">
                <div class="adw-row">
                    ...
                    <select
                        class="adw-select"
                        id="kb-preset"
                        onchange={() => applyKbPreset(this.value)}
                    >
            ...
            <div class="adw-group">
                <div class="adw-row">
                    ...
                    <button class="adw-button destructive" onclick={clearDataCache}
                        >Clear Cache</button
                    >
                </div>
            </div>
